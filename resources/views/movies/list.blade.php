        ...
        <div class="card-body">
            <form action="{{ route('movies.list') }}" method="GET" class="row g-3 align-items-end">
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control bg-secondary text-white border-dark" id="search" name="search" placeholder="e.g., Inception" value="{{ $search ?? '' }}">
                </div>
                <div class="col-md-4">
                    ...
                    <select class="form-select bg-secondary text-white border-dark" id="genre" name="genre">
                        ...
                        @foreach ($genres as $genre)
                            <option value="{{ $genre->code }}" {{ ($selectedGenre ?? '') == $genre->code ? 'selected' : '' }}>
                                {{ $genre->name }}
                            </option>
                        @endforeach
                    </select>
